...
handleSessionTerminated(lineObj) {
    ...
    try {
        ...
        if (callRecord.caller !== 'unknown' || callRecord.callee !== 'unknown') {
            fetch("https://bkpmanual.bitnexdial.com/api/save-call", {
                method: "POST",
...
function connectSocket(retry = 0) {
    ...
    window.socket = io("https://bkpmanual.bitnexdial.com", {
...
function FetchSMSHistory(buddyObj) {
    ...
    fetch("https://bkpmanual.bitnexdial.com/sms-history", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
            number: senderNumber,
...
function FetchCallHistoryOnLogin(done) {
    ...
    fetch("https://bkpmanual.bitnexdial.com/api/call-history?number=" + myPhone)
        .then(res => res.json())
        .then(data => {
            if (!Array.isArray(data)) {
...
function startSessionPing() {
    ...
    setInterval(() => {
        fetch("https://bkpmanual.bitnexdial.com/api/ping", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
...
function UpdateMySMSCounter() {
    ...
    fetch(`https://bkpmanual.bitnexdial.com/api/sms-counter-user?number=${myNumber}&_=${ts}`, {
        method: 'GET',
        credentials: 'include',
        cache: 'no-store'
    })
...
function SendUnifiedMessage(buddy, { text = "", imageDataUrl = "" } = {}) {
    ...
    if (imageDataUrl) {
        ...
        fetch("https://bkpmanual.bitnexdial.com/upload", {
            method: "POST",
            body: formData
        })
            .then(res => res.json())
...
function loadMoreMessages(buddyId, offset = 0) {
    ...
    return fetch("https://bkpmanual.bitnexdial.com/sms-history-paginated", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
function SendFileDataMessage(buddy, FileDataUrl, fileName, fileSize) {
    ...
    $.ajax({
        type: 'POST',
        url: 'https://bkpmanual.bitnexdial.com/upload',
        data: formData,
        contentType: false,
        processData: false,
        xhr: function () {
